...
// commitChanges commits the current changes to git, including untracked files.
func (l *Loop) commitChanges(ctx context.Context, task *Task) error {
	...
	// Use -c safe.directory inline to handle containers creating files owned
	// by a different UID (e.g., agent UID 1000) than the host user. This avoids
	// "dubious ownership" errors without mutating global git config.
	safeCfg := fmt.Sprintf("safe.directory=%s", l.projectPath)

	// Stage all changes including untracked files. The -A flag stages new,
	// modified, and deleted files across the entire working tree.
	addCmd := exec.CommandContext(ctx, "git", "-c", safeCfg, "add", "-A")
	addCmd.Dir = l.projectPath
	var addStderr strings.Builder
	addCmd.Stderr = &addStderr
	if err := addCmd.Run(); err != nil {
		return fmt.Errorf("git add -A: %w (stderr: %s)", err, addStderr.String())
	}
	if addStderr.Len() > 0 {
		l.logger.Warn("git add warnings", "stderr", addStderr.String())
	}

	// Check if there are staged changes to commit.
	statusCmd := exec.CommandContext(ctx, "git", "-c", safeCfg, "status", "--porcelain")
	statusCmd.Dir = l.projectPath
	var statusStderr strings.Builder
	statusCmd.Stderr = &statusStderr
	output, err := statusCmd.Output()
	if err != nil {
		return fmt.Errorf("git status: %w (stderr: %s)", err, statusStderr.String())
	}
	if len(output) == 0 {
		l.logger.Debug("no changes to commit")
		return nil
	}

	stagedFiles := strings.Split(strings.TrimSpace(string(output)), "\n")
	l.logger.Debug("staged changes", "files", strings.TrimSpace(string(output)))

	message := fmt.Sprintf("feat: %s\n\nTask ID: %s\nIteration: %d\n\nGenerated by agentbox Ralph loop",
		task.Title, task.ID, l.iteration)

	commitCmd := exec.CommandContext(ctx, "git", "-c", safeCfg, "commit", "-m", message)
	commitCmd.Dir = l.projectPath
	var commitStderr strings.Builder
	commitCmd.Stderr = &commitStderr
	if err := commitCmd.Run(); err != nil {
		return fmt.Errorf("git commit: %w (stderr: %s)", err, commitStderr.String())
	}

	l.logger.Info("committed changes",
		"task", task.ID,
		"iteration", l.iteration,
